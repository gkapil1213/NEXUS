...
export class RecoveryStore {
  ...
  listPolicies(): RecoveryPolicy[] {
    return this.db
      .prepare("SELECT * FROM recovery_policies ORDER BY created_at DESC")
      .all()
      .map(this.mapPolicy);
  }
  ...
  addAttempt(attempt: RecoveryAttemptRecord): void {
    this.db
      ...
      .run(
        ...
        attempt.decision,
        attempt.status,
        attempt.verificationResult === undefined ? null : attempt.verificationResult ? 1 : 0,
  ...
  getAttemptByIdempotencyKey(key: string): RecoveryAttemptRecord | undefined {
    const row = this.db
      .prepare("SELECT * FROM recovery_attempts WHERE idempotency_key = ?")
      .get(key);
  ...
  listAttemptsForIncident(incidentId: string): RecoveryAttemptRecord[] {
    const rows = this.db
      .prepare("SELECT * FROM recovery_attempts WHERE incident_id = ? ORDER BY attempt_number ASC")
      .all(incidentId);

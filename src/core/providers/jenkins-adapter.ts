...
export class JenkinsAdapter implements CICDProvider {
  id = "jenkins";
  constructor(private config: { baseUrl?: string; username?: string; token?: string } = {}) {}
  ...
  validateRequest(request: any): { valid: boolean; errors: string[] } {
    ...
    if (!this.config.token && !this.config.username) errors.push("Jenkins credentials not configured");
  ...
  async trigger(request: any): Promise<{ externalRunId: string }> {
    ...
    throw new Error("Jenkins execution not available in this environment (no credentials or network)");
  }

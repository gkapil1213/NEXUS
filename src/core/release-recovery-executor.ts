...
export class ReleaseRecoveryExecutor {
  ...
  async runOnce(now = Date.now()): Promise<RecoveryRunReport> {
    ...
    await svc.events.emit({
      type: "release.recovery.started",
      source: "ReleaseRecoveryExecutor",
      payload: { scanned: recoverable.length, workerId: this.deps.workerId },
    });
    for (const intent of recoverable) {
      try {
        ...
        await svc.events.emit({
          type: "release.recovery.error",
          source: "ReleaseRecoveryExecutor",
          execution_id: intent.executionId,
          payload: { intentKey: intent.intentKey, error: reason },
        });
      }
    }
    ...
    if (this.deps.reconciler) {
      ...
      for (const status of terminalStatuses) {
        for (const intent of intents.listByStatus(status)) {
          const kind =
            (intent as unknown as { intentKind?: string }).intentKind ??
            "DEPLOY";
          ...
          const reconcileLease = intents.acquireLease(
            intent.intentKey,
            this.deps.workerId,
          ...
          if (!reconcileLease.acquired) {
            report.leaseHeld++;
            await svc.events.emit({
              type: "release.recovery.lease.held",
              source: "ReleaseRecoveryExecutor",
              execution_id: intent.executionId,
              payload: {
                intentKey: intent.intentKey,
                holder: reconcileLease.holder,
                expiresAt: reconcileLease.expiresAt,
                phase: "reconcile",
              },
            });
            continue;
          ...
          try {
            ...
            report.blockedReasons.push({
              intentKey: intent.intentKey,
              reason: "reconciliation failed: " + reason,
            });
            await svc.events.emit({
              type: "release.recovery.error",
              source: "ReleaseRecoveryExecutor",
  ...
  private async dispatch(
    ...
    switch (plan.action) {
      ...
      case "ALREADY_BLOCKED":
        report.skipped++;
        return;
      case "RESUME_FROM_INTENT":
        await this.resumeFromIntent(intent, report);
        return;
      case "RESUME_VERIFICATION":
        await this.resumeVerification(intent, report);
        return;
      case "MARK_FAILED_AND_ROLLBACK":
        await this.markFailedAndRollback(intent, report);
        return;
      case "RESUME_ROLLBACK":
        await this.resumeRollback(intent, report);
        return;
      case "RECOVERY_REQUIRED":
        await this.handleRecoveryRequired(intent, plan, report);
        return;
      default: {
        const _ex: never = plan.action;
        void _ex;
        report.blocked++;
        report.blockedReasons.push({
          intentKey: intent.intentKey,
  ...
  private async resumeFromIntent(
    intent: ReleaseDeploymentIntent,
    report: RecoveryRunReport,
  ): Promise<void> {
    const { intents, svc } = this.deps;
    if (intent.status === "AUTHORIZED") {
      if (!this.isImmutableComplete(intent)) {
        await this.blockIntent(
          intent,
          "authorized intent missing immutable fields",
          report,
        );
        return;
      }
      intents.transition(intent.intentKey, "DEPLOYMENT_INTENT_CREATED", {
        recoveryReason: "recovery: AUTHORIZED -> DEPLOYMENT_INTENT_CREATED",
      });
      report.acted++;
      await svc.events.emit({
        type: "release.recovery.advanced",
        source: "ReleaseRecoveryExecutor",
        execution_id: intent.executionId,
        payload: {
          intentKey: intent.intentKey,
          from: "AUTHORIZED",
          to: "DEPLOYMENT_INTENT_CREATED",
        },
      });
      return;
    }
    if (intent.status === "DEPLOYMENT_INTENT_CREATED") {
      if (!intent.projectId) {
        await this.blockIntent(intent, "projectId required to deploy", report);
        return;
      }
      if (!this.isImmutableComplete(intent)) {
        await this.blockIntent(
          intent,
          "intent missing immutable fields",
          report,
        );
        return;
      }
      ...
      try {
        ...
        if (!fresh || fresh.status !== "DEPLOYMENT_INTENT_CREATED") {
          report.skipped++;
          return;
        }
        intents.transition(fresh.intentKey, "DEPLOYING", {
          recoveryReason:
            "recovery: deploying under lease " + this.deps.workerId,
        });
        const outcome = await this.deps.orchestrator.deploy(
          this.toDeploymentRequest(fresh),
        );
    ...
    await this.blockIntent(
      intent,
      "resume_from_intent but status " + intent.status,
      report,
    );
  }
  ...
  private async resumeVerification(
    ...
    if (!intent.projectId) {
      await this.blockIntent(
        intent,
        "projectId required to resume verification",
        report,
      );
      return;
    }
    if (!intent.deploymentId) {
      await this.blockIntent(
        intent,
        "no deploymentId to resume verification",
        report,
      );
      return;
    }
    ...
    try {
      ...
      if (
        !fresh ||
        (fresh.status !== "HEALTH_CHECKING" && fresh.status !== "SMOKE_TESTING")
      ) {
        report.skipped++;
        return;
      }
      ...
      if (inspection.verdict === "BLOCKED") {
        await this.markRecoveryRequired(
          fresh,
          "inspection blocked: " + inspection.reason,
        );
        report.blocked++;
        return;
      }
      if (inspection.verdict === "MISSING") {
        await this.markRecoveryRequired(
          fresh,
          "container missing on resume verification",
        );
        report.blocked++;
        return;
      }
      if (inspection.verdict === "IDENTITY_MISMATCH") {
        intents.transition(fresh.intentKey, "VERIFICATION_FAILED", {
          failureReason:
            "identity mismatch on resume: expected " +
            inspection.expectedImageId +
            " got " +
            inspection.runningImageId,
        });
        report.acted++;
        return;
      }
      if (!inspection.hostPort) {
        await this.markRecoveryRequired(fresh, "no host port on inspect");
        report.blocked++;
        return;
      }
      const stagingUrl = "http://127.0.0.1:" + inspection.hostPort;
      const smokeResult = await this.deps.smoke.run({
        staging_url: stagingUrl,
        execution_id: fresh.executionId,
      });
      if (smokeResult.verdict === "PASS") {
        intents.transition(fresh.intentKey, "KNOWN_GOOD", {
          deploymentId: fresh.deploymentId,
        });
        report.acted++;
        await this.deps.svc.events.emit({
          type: "release.recovery.known_good",
          source: "ReleaseRecoveryExecutor",
          execution_id: fresh.executionId,
          payload: {
        ...
        intents.transition(fresh.intentKey, "VERIFICATION_FAILED", {
          failureReason: "smoke failed on resume",
        });
        report.acted++;
      }
    } finally {
  ...
  private async markFailedAndRollback(
    ...
    const { intents, svc } = this.deps;
    if (!intent.projectId) {
      await this.blockIntent(intent, "projectId required for rollback", report);
      return;
    }
    const lease = intents.acquireLease(
      intent.intentKey,
    ...
    try {
      const fresh = intents.get(intent.intentKey);
      if (!fresh || fresh.status !== "VERIFICATION_FAILED") {
        report.skipped++;
        return;
      }
      intents.transition(fresh.intentKey, "ROLLING_BACK", {
        recoveryReason: "recovery: verification failed -> rollback",
      });
      if (!this.deps.rollback) {
        intents.transition(fresh.intentKey, "FAILED", {
          failureReason: "verification failed; rollback delegate unavailable",
        });
        report.blocked++;
        report.blockedReasons.push({
          intentKey: fresh.intentKey,
          reason: "rollback delegate unavailable",
        });
        await svc.events.emit({
          type: "release.recovery.rollback.unavailable",
          source: "ReleaseRecoveryExecutor",
          execution_id: fresh.executionId,
          payload: { intentKey: fresh.intentKey },
        });
        return;
      }
      ...
      intents.transition(
        fresh.intentKey,
        result.status === "COMPLETED" ? "FAILED" : "BLOCKED",
        { deploymentId: result.deploymentId, failureReason: result.message },
      );
      if (result.status === "COMPLETED") report.acted++;
      else report.blocked++;
      await svc.audit.record({
        actor: this.deps.workerId,
        action: "release.recovery.rollback",
        resource_type: "release_deployment_intent",
        resource_id: fresh.intentKey,
        result: result.status === "COMPLETED" ? "ok" : "blocked",
  ...
  private async resumeRollback(
    ...
    if (!intent.projectId) {
      await this.blockIntent(
        intent,
        "projectId required to resume rollback",
        report,
      );
      return;
    }
    ...
    try {
      ...
      if (inspection.verdict === "BLOCKED") {
        ...
        report.blockedReasons.push({
          intentKey: fresh.intentKey,
          reason: "rollback resume: inspection blocked",
        });
        return;
      }
      if (inspection.verdict === "MISSING") {
        await this.markRecoveryRequired(
          fresh,
          "rollback resume: container missing",
        );
        report.blocked++;
        return;
      }
      if (inspection.verdict === "MATCHES_INTENT") {
        const isRollbackKind =
          ((fresh as any).intentKind ?? "DEPLOY") === "ROLLBACK";
        if (isRollbackKind) {
          // Phase 119: target image already running. Do NOT invoke rollback again.
          if (!this.deps.verifyRecoveredRollback) {
            await this.markRecoveryRequired(
              fresh,
              "target image already active; recovery verification delegate unavailable",
            );
            report.blocked++;
            return;
          }
          ...
          if (!inspection.hostPort) {
            await this.markRecoveryRequired(
              fresh,
              "target image active but no mapped host port on inspect",
            );
            report.blocked++;
            return;
          }
          ...
          try {
            ...
            await this.markRecoveryRequired(
              fresh,
              "recovery verification threw: " + errMsg,
            );
            report.blocked++;
            return;
          }
          ...
          if (ver.status === "VERIFIED") {
            ...
            intents.transition(fresh.intentKey, "FAILED", {
              recoveryReason: "rollback verified after crash recovery",
            });
            report.acted++;
            await this.deps.svc.events.emit({
          ...
          if (ver.status === "BLOCKED") {
            ...
            await this.markRecoveryRequired(
              fresh,
              "recovery verification blocked: " + ver.message,
            );
            report.blocked++;
            return;
          }
        ...
        if (!this.deps.rollback) {
          intents.transition(fresh.intentKey, "BLOCKED", {
            failureReason: "rollback in flight; delegate unavailable",
          });
          report.blocked++;
          return;
        }
      ...
      intents.transition(fresh.intentKey, "FAILED", {
        failureReason: "rollback already completed; terminal",
      });
      report.skipped++;
    } finally {
      intents.releaseLease(intent.intentKey, this.deps.workerId);
  ...
  private async handleRecoveryRequired(
    intent: ReleaseDeploymentIntent,
    plan: RecoveryPlan,
    report: RecoveryRunReport,
  ): Promise<void> {
    ...
    try {
      ...
      const inspection = await inspectIntentContainer(fresh, this.deps.docker);
      if (inspection.verdict === "MATCHES_INTENT") {
        intents.transition(fresh.intentKey, "HEALTH_CHECKING", {
          recoveryReason:
            "recovery: container matches intent -> HEALTH_CHECKING",
        });
        report.acted++;
        return;
      }
      if (inspection.verdict === "IDENTITY_MISMATCH") {
        intents.transition(fresh.intentKey, "VERIFICATION_FAILED", {
          failureReason: "recovery: container identity mismatch on resume",
        });
        report.acted++;
        return;
      }
      if (inspection.verdict === "MISSING") {
        await this.markRecoveryRequired(
          fresh,
          "container missing on recovery; manual review required",
        );
        report.blocked++;
        return;
      }
      ...
      report.blockedReasons.push({
        intentKey: fresh.intentKey,
        reason: "inspection blocked: " + inspection.reason,
      });
    } finally {
      intents.releaseLease(intent.intentKey, this.deps.workerId);
    }
  ...
  private async recordDeploymentOutcome(
    intent: ReleaseDeploymentIntent,
    outcome: CanonicalDeploymentOutcome,
    report: RecoveryRunReport,
  ): Promise<void> {
    const { intents, svc } = this.deps;
    ...
    if (status === "KNOWN_GOOD") {
      ...
      await svc.events.emit({
        ...
        source: "ReleaseRecoveryExecutor",
        execution_id: intent.executionId,
        payload: { intentKey: intent.intentKey, deploymentId },
      });
      return;
    }
    if (status === "BLOCKED") {
      intents.transition(intent.intentKey, "BLOCKED", {
        deploymentId,
        failureReason: "deployment blocked",
      });
      report.blocked++;
      return;
    }
  ...
  private isImmutableComplete(intent: ReleaseDeploymentIntent): boolean {
    return !!(
      ...
      intent.commitSha &&
      intent.environment &&
      intent.imageRepository &&
      intent.imageTag &&
      intent.imageDigest &&
      intent.containerName &&
      intent.containerPort > 0
    );
  }
  ...
  private async blockIntent(
    ...
    report.blockedReasons.push({ intentKey: intent.intentKey, reason });
    await this.deps.svc.events.emit({
      type: "release.recovery.blocked",
      source: "ReleaseRecoveryExecutor",
      execution_id: intent.executionId,
      payload: { intentKey: intent.intentKey, reason },
    });
  }
  ...
  private async markRecoveryRequired(
    ...
    this.deps.intents.transition(intent.intentKey, "RECOVERY_REQUIRED", {
      recoveryReason: reason,
    });
    await this.deps.svc.audit.record({
      actor: this.deps.workerId,
      action: "release.recovery.required",
      resource_type: "release_deployment_intent",

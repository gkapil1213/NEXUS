...
export class GitHubActionsGenerator {
  ...
  generate(plan: PipelinePlan, repo: string): PipelineConfig {
    const deployEnabled = plan.steps.some(
      (s) => /publish|push|registry/i.test(s.type) || /publish|push|registry/i.test(s.name),
    );
    const lines: string[] = [];
    ...
    lines.push("    inputs:");
    lines.push("      execution_id:");
    lines.push("        description: NEXUS execution id");
    lines.push("        required: false");
    lines.push('        default: ""');
    lines.push("      attempt:");
    lines.push("        description: CI attempt");
    lines.push("        required: false");
    lines.push('        default: "1"');
    if (deployEnabled) {
      lines.push("env:");
      lines.push("  NEXUS_REGISTRY: " + "${{ vars.NEXUS_REGISTRY }}");
      lines.push("  NEXUS_IMAGE_REPO: " + "${{ vars.NEXUS_IMAGE_REPO }}");
      lines.push("  NEXUS_IMAGE_TAG: " + "sha-${{ github.sha }}");
      lines.push("  NEXUS_EXECUTION_ID: " + "${{ inputs.execution_id }}");
      lines.push("  NEXUS_PROJECT_ID: " + "${{ inputs.project_id }}");
    }
    lines.push("jobs:");
    lines.push("  ci:");
    ...
    if (deployEnabled) {
      lines.push("      - name: Log in to container registry");
      lines.push("        uses: docker/login-action@v3");
      lines.push("        with:");
      lines.push("          registry: " + "${{ env.NEXUS_REGISTRY }}");
      lines.push("          username: " + "${{ secrets.NEXUS_REGISTRY_USERNAME }}");
      lines.push("          password: " + "${{ secrets.NEXUS_REGISTRY_PASSWORD }}");
      lines.push("      - name: Set up Docker Buildx");
      lines.push("        uses: docker/setup-buildx-action@v3");
      lines.push("      - name: Build and push immutable image");
      lines.push("        id: nexus_build");
      lines.push("        uses: docker/build-push-action@v5");
      lines.push("        with:");
      lines.push("          context: .");
      lines.push("          push: true");
      lines.push("          tags: " + "${{ env.NEXUS_REGISTRY }}/${{ env.NEXUS_IMAGE_REPO }}:${{ env.NEXUS_IMAGE_TAG }}");
      lines.push("      - name: Write nexus-image-digest.json");
      lines.push("        shell: bash");
      lines.push("        run: |");
      lines.push("          set -euo pipefail");
      lines.push('          DIGEST="${{ steps.nexus_build.outputs.digest }}"');
      lines.push('          REF="${{ env.NEXUS_REGISTRY }}/${{ env.NEXUS_IMAGE_REPO }}@${DIGEST}"');
      lines.push("          cat > nexus-image-digest.json <<JSON");
      lines.push("          {");
      lines.push('            "schema_version": 1,');
      lines.push('            "execution_id": "${{ env.NEXUS_EXECUTION_ID }}",');
      lines.push('            "project_id": "${{ env.NEXUS_PROJECT_ID }}",');
      lines.push('            "repository": "${{ github.repository }}",');
      lines.push('            "commit_sha": "${{ github.sha }}",');
      lines.push('            "ci_provider": "github-actions",');
      lines.push('            "external_run_id": "${{ github.run_id }}",');
      lines.push('            "image_repository": "${{ env.NEXUS_REGISTRY }}/${{ env.NEXUS_IMAGE_REPO }}",');
      lines.push('            "image_tag": "${{ env.NEXUS_IMAGE_TAG }}",');
      lines.push('            "image_digest": "${DIGEST}",');
      lines.push('            "immutable_reference": "${REF}",');
      lines.push('            "published_at": "$(date -u +%Y-%m-%dT%H:%M:%SZ)",');
      lines.push('            "build_workflow": ".github/workflows/nexus-ci.yml"');
      lines.push("          }");
      lines.push("          JSON");
      lines.push("      - name: Upload nexus-image-digest.json");
      lines.push("        uses: actions/upload-artifact@v4");
      lines.push("        with:");
      lines.push("          name: nexus-image-digest.json");
      lines.push("          path: nexus-image-digest.json");
      lines.push("          if-no-files-found: error");
      lines.push("          retention-days: 90");
    }

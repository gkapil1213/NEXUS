...
async function main() {
  console.log('Starting Phase 9 Pass 1 verification...');
  const evidence: any = {
    capabilities: [],
    baseline: null,
    performance: null,
  ...
  try {
    ...
    const perfMetrics = await perfAgent.run(`${baseUrl}/health`, {
      concurrency: 5,
      requests: 200,          // use requests instead of duration for stability
      warmupMs: 500
    });
  ...
  try {
    ...
    console.log('Load test complete:', loadResults);
  } catch (err) {
    console.error('Load test failed:', err);
  }

  // Stress test (use fewer levels to keep runtime short)
  try {
  ...
  try {
    ...
    console.log('Failure injected and restored:', injectionResult);
  } catch (err) {
  ...
  const evidencePath = path.join(process.cwd(), 'phase9-pass1-evidence.json');

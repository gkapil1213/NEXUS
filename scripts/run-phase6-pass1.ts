...
async function main() {
  ...
  for (const cap of capabilities) {
    if (["terraform", "aws_cli", "docker_cli", "docker_daemon", "node", "npm"].includes(cap.name)) {
  ...
  const evidence = {
    ...
    capabilities: capabilities.filter(c => ["terraform", "aws_cli", "docker_cli", "docker_daemon", "node", "npm"].includes(c.name)),
    terraform: {

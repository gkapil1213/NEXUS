...
function insertMetric(m: any) { const id = randomUUID(); db.prepare(`INSERT INTO observability_metrics (id, timestamp, service, environment, metric_name, metric_value, unit, source, metadata, execution_id, trace_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, m.timestamp, m.service, m.environment, m.metric_name, m.metric_value, m.unit, m.source, JSON.stringify(m.metadata || {}), m.execution_id, m.trace_id); }
function insertLog(l: any) { const id = randomUUID(); db.prepare(`INSERT INTO observability_logs (id, timestamp, level, service, message, execution_id, metadata, trace_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(id, l.timestamp, l.level, l.service, redactSecrets(l.message), l.execution_id, JSON.stringify(l.metadata || {}), l.trace_id); }
function insertTrace(t: any) { const id = randomUUID(); db.prepare(`INSERT INTO local_traces (id, trace_id, service, operation, started_at, duration_ms, status, attributes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(id, t.trace_id, t.service, t.operation, t.started_at, t.duration_ms, t.status, JSON.stringify(t.attributes || {})); }
function insertEvent(type: string, incidentId: string, extra: any = {}) { const id = randomUUID(); db.prepare(`INSERT INTO events (id, type, timestamp, data) VALUES (?, ?, ?, ?)`).run(id, type, new Date().toISOString(), JSON.stringify({ incident_id: incidentId, ...extra })); }
function insertAudit(action: string, resource: string, result: string, incidentId: string, metadata: any = {}) { const id = randomUUID(); db.prepare(`INSERT INTO audits (id, actor, action, resource, result, metadata, timestamp, reliability_run_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(id, 'system', action, resource, result, JSON.stringify(metadata), new Date().toISOString(), null); }

async function main() {
  console.log('Phase 10 Pass 4 verification started');
  const evidence: any = {
  ...
  let mode = 'normal';
  fixtureApp.get('/health', (req, res) => { if (mode === 'error') return res.status(503).send('Unavailable'); res.json({ status: 'ok' }); });
  fixtureApp.post('/toggle', (req, res) => { mode = req.body.mode; res.json({ mode }); });
  ...
  const traceId = randomUUID(); const incidentId = randomUUID(); const alertFingerprint = 'fixture:local:HTTP_503:high';
  ...
  fs.writeFileSync(path.join(process.cwd(), 'phase10-pass4-evidence.json'), JSON.stringify(evidence, null, 2));
